...
class ExampleConsumer(object):
    ...
    def on_connection_closed(self, connection, reply_code, reply_text):
        ...
        if self._closing:
            ...
            if self._reconnect_times > 0:
                self._logger.warning('[OMQS][AsyncConsumer] Connection closed. '
                                     '[%d]Reopening in %d seconds: (reply code:%s) %s',
    ...
    def on_queue_declareok(self, method_frame):
        ...
        """
        self._logger.info('[OMQS][AsyncConsumer] Binding %s to %s with %s',
    ...
    def start_consuming(self):
        ...
        """
        self._logger.info('[OMQS][AsyncConsumer]Issuing consumer related RPC commands')
    ...
    def stop_consuming(self):
        ...
        """
        if self._channel:
            self._logger.info('[OMQS][AsyncConsumer] Sending a Basic.Cancel RPC command to RabbitMQ')
    ...
    def stop(self):
        ...
        """
        self._logger.info('[OMQS][AsyncConsumer] Stopping')
        self._closing = True
        self.stop_consuming()
        self._connection.ioloop.start()
        self._logger.info('[OMQS][AsyncConsumer] Stopped')

    def close_connection(self):
        """This method closes the connection to RabbitMQ."""
        self._logger.info('[OMQS][AsyncConsumer] Closing connection')
        self._connection.close()


def main():
    #logging.basicConfig(level=logging.INFO, format=LOG_FORMAT)
    #example = ExampleConsumer('amqp://artcm:111@192.168.0.197:5672/%2F')
    example = ExampleConsumer(amqp_url='amqp://artcm:111@192.168.1.107:5672/%2F',routing_key=['debug', 'info', 'error'])
    try:
        example.run()
